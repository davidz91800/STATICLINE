...
const dom = {
    ...
    infoContentContainer: document.getElementById('info-content-container')
};
...
function validateInputs({ H, securityLevel, parachuteType, longWind, crossWind, hasVisualRefs }) {
    ...
    if (hLimits) {
        if (H < hLimits.min) messages.push({ text: `ALERTE HAUTEUR: ${H.toFixed(0)}m < min ${hLimits.min}m.`, type: 'alert-error' });
        if (H > hLimits.max) messages.push({ text: `ALERTE HAUTEUR: ${H.toFixed(0)}m > max ${hLimits.max}m.`, type: 'alert-error' });
    }
...
// NOUVEAU: Fonction pour activer les accordéons dans la section d'info
function initializeInfoAccordions() {
    const headers = document.querySelectorAll('.info-accordion-header');
...
// MODIFIÉ : Fonction pour charger le contenu et initialiser les accordéons
async function loadInfoContent() {
    if (!dom.infoContentContainer) return;
    try {
        const response = await fetch('info-content.html');
        if (!response.ok) {
            throw new Error(`Le fichier d'aide n'a pas pu être chargé (statut: ${response.status})`);
        }
        const htmlContent = await response.text();
        dom.infoContentContainer.innerHTML = htmlContent;
        initializeInfoAccordions(); // On active les accordéons une fois le contenu chargé
    } catch (error) {
        console.error("Erreur lors du chargement de la section d'information:", error);
        dom.infoContentContainer.innerHTML = `<p class="alert alert-error">Impossible de charger le contenu de l'aide. Vérifiez votre connexion ou que le fichier <code>info-content.html</code> est présent.</p>`;
    }
}
